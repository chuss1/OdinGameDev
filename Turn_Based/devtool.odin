...
list_dev_tools :: proc() {
    result : i32 = rl.GuiMessageBox((rl.Rectangle){85,70,250,100}, "#191#Available Dev Tools", "Here is all of the available tools", "UNIT_CREATE;ITEM_CREATE")

    switch result {
        case 0 : //The Exit Button
...
// Have to draw dropdown boxes last so they overwrite the options below it
draw_unit_create :: proc() {
    ...
    if rl.GuiValueBox((rl.Rectangle){120,168,24,24}, "ROW: ", &value_x, 0, 100, value_x_edit) == 1{
        value_x_edit = !value_x_edit
    }
    if rl.GuiValueBox((rl.Rectangle){180,168,24,24}, "COL: ", &value_z, 0, 100, value_z_edit) == 1{
    ...
    if confirm_button {
        ...
        unit_create(checkbox_is_friendly, DROPDOWN_VALUE, (GridUnit){int(VALUE_X), int(VALUE_Z)})
